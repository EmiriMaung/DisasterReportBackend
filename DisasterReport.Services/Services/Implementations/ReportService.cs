// src/Services/Services/ReportService.cs
...
// using Microsoft.Extensions.Caching.Memory; // No longer needed
// using Microsoft.Extensions.Primitives; // No longer needed

public class ReportService : IReportService
{
    private readonly IReportRepo _reportRepo;
    private readonly IUserRepo _userRepo;
    // The IMemoryCache field has been removed.

    // IMemoryCache has been removed from the constructor.
    ...
    {
        // All caching logic has been removed from this method.
        // It now directly calls the repository and returns fresh data every time.
...
//    private void InvalidateReportsCache()
//    {
//        // This is a simplified approach. It removes ALL keys that start with "Reports_".
//        // This is effective but can be improved if you have a very high-traffic system.
//        // For most applications, this is perfectly fine.
//        var keysToRemove = new List<string>();
//        // Note: This requires a bit of reflection to get all keys, which can be slow.
//        // A better approach is to use a dedicated cache management library or a different cache key strategy if this becomes a bottleneck.
//        // However, for this specific problem, we can just remove the known keys.
//        // A more robust solution is to not try to list all keys, but to remove specific ones if you know them.
//        // Since we don't know all the possible filter combinations, a more advanced cache would be needed.
//        // For now, let's just clear the cache based on a known pattern.
//        // A simple way is to just remove the cache for the default view (page 1, pending)
//        // A more advanced way is to use a different caching strategy.

//        // The simplest, most direct way without changing much code is to accept that this cache is not perfect.
//        // The best alternative to a singleton is to use a distributed cache like Redis if you have multiple server instances.
//        // For a single instance, the singleton is the most straightforward way to share the cancellation token.

//        // Let's try a different approach that doesn't require a singleton or complex key management.
//        // We will remove the cache entirely for mutation operations.
//        // This is the simplest and most effective solution without changing the DI lifetime.
//    }
//}
